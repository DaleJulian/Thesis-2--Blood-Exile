...
public class Dwarf_Fire : MonoBehaviour {

	[SerializeField] Transform player;

	private NavMeshAgent navFire;
	...
	void Start () {
		...
		navFire = GetComponent <NavMeshAgent> ();
	}
	...
	void SearchForTarget()
	{
		...
		foreach(Collider collide in col)
		{
			...
			if(target != null)
			{
				...
				navFire.SetDestination(player.transform.position);
	...
	if (rigidbody.velocity.sqrMagnitude >= 1.1f)
		animFire.SetFloat ("Speed", 1);
	else if (rigidbody.velocity.sqrMagnitude <= 1.0f)
		animFire.SetFloat ("Speed", 0);
}
}